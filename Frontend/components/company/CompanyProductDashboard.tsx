...
const CompanyProductsDashboard = () => {
  ...
  const products = useMemo(() => {
    ...
    return allProducts.filter(
      (product: Product) => product.companyId === user.uid
    );
  }, [allProducts, user]);
  ...
  return (
    <View className="flex-1">
      <TouchableOpacity
        ...
        className="bg-br p-3 rounded-lg my-4"
      >
        ...
        >
          ...
          {modelFile && (
            <View className="mt-2 h-64 border border-g-200 rounded-lg overflow-hidden">
              <Text className="text-center p-2 bg-g-100 text-bl">
                {modelFile.name}
              </Text>
              <ModelViewer modelUri={modelFile.uri} style={{ flex: 1 }} />
            </View>
          )}
          <Text className="text-bl font-semibold mt-4 mb-2">
            Dimensions (cm)
...
export default CompanyProductsDashboard;